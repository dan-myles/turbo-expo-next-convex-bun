...
import type { PublicActionCtx, PublicMutationCtx, PublicQueryCtx } from "#backend/lib/middleware"

export class Query extends Context.Tag("Query")<Query, PublicQueryCtx>() {
  static live = (ctx: PublicQueryCtx) => Layer.succeed(Query, ctx)
}

export class Mutation extends Context.Tag("Mutation")<Mutation, PublicMutationCtx>() {
